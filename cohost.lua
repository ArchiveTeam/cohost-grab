...
wget.callbacks.download_child_p = function(urlpos, parent, depth, start_url_parsed, iri, verdict, reason)
  ...
  if current_item_type == "userfix1" or current_item_type == "url" then
    return false
  end
...
wget.callbacks.httploop_result = function(url, err, http_stat)
  ...
  if status_code ~= 200 and status_code ~= 404
    ...
    and not ((status_code == 400 or status_code == 403 or status_code == 500) and url["url"]:match("^https?://[a-z%-]+%.cohostcdn%.org/.*"))
...
wget.callbacks.write_to_warc = function(url, http_stat)
  ...
  if (string.match(url["url"], "^https?://cohost%.org/") or string.match(url["url"], "^https?://[^%.]+%.cohost%.org/") or string.match(url["url"], "^https?://[^%.]+%.cohostcdn%.org/"))
          ...
          and not ((http_stat["statcode"] == 400 or http_stat["statcode"] == 403 or http_stat["statcode"] == 500) and url["url"]:match("^https?://[a-z%-]+%.cohostcdn%.org/.*"))
